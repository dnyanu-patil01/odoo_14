...
class AccountMove(models.Model):
    ...
    def _get_filtered_seller_invoices_record(self):
        action = self.env["ir.actions.actions"]._for_xml_id("seller_management.action_sellers_invoices")
        if self.env.user.has_group('seller_management.group_sellers_management_manager'):
            action['domain'] = [('move_type','=','out_invoice'),('seller_id','!=',False)]
        elif self.env.user.has_group('seller_management.group_sellers_management_user'):
            action['domain'] = [
                ('seller_id', '=', self.env.user.partner_id.id),
                ('move_type','=','out_invoice'),('seller_id','!=',False)
            ]
        else:
            action['domain'] = [('move_type','=','out_invoice'),('seller_id','!=',False)]
        action['context'] = {'default_move_type':'out_invoice','default_seller_id':self.env.user.partner_id.id}
        return action