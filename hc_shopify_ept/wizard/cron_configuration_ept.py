...
class ShopifyCronConfigurationEpt(models.TransientModel):
    ...
    def update_export_stock_cron_field(self, instance):
        ...
        try:
            export_inventory_stock_cron_exist = instance and self.env.ref(
                'hc_shopify_ept.ir_cron_shopify_auto_export_inventory_instance_%d' % instance.id)
        except:
    ...
    def update_import_order_cron_field(self, instance):
        ...
        try:
            import_order_cron_exist = instance and self.env.ref(
                'hc_shopify_ept.ir_cron_shopify_auto_import_order_instance_%d' % instance.id)
        except:
    ...
    def update_order_status_cron_field(self, instance):
        ...
        try:
            update_order_status_cron_exist = instance and self.env.ref(
                'hc_shopify_ept.ir_cron_shopify_auto_update_order_status_instance_%d' % instance.id)
        except:
    ...
    def update_payout_report_cron_field(self, instance):
        ...
        try:
            payout_report_cron_exist = instance and self.env.ref(
                'hc_shopify_ept.ir_cron_auto_import_payout_report_instance_%d' % instance.id)
        except:
            payout_report_cron_exist = False
        try:
            auto_process_bank_statement_cron_exist = instance and self.env.ref(
                'hc_shopify_ept.ir_cron_auto_process_bank_statement_instance_%d' % instance.id)
        except:
        ...
        if self._context.get('is_calling_from_onboarding_panel', False):
            ...
            if instance:
                action = self.env["ir.actions.actions"]._for_xml_id(
                    "hc_shopify_ept.shopify_onboarding_confirmation_wizard_action")
    ...
    def setup_shopify_inventory_export_cron(self, instance):
        ...
        if self.shopify_stock_auto_export:
            ...
            if cron_exist:
                ...
                core_cron = self.check_core_shopify_cron("hc_shopify_ept.ir_cron_shopify_auto_export_inventory")

                name = instance.name + ' : ' + core_cron.name
                vals.update({'name': name})
                new_cron = core_cron.copy(default=vals)
                self.env['ir.model.data'].create({'module': 'hc_shopify_ept',
    ...
    def setup_shopify_import_order_cron(self, instance):
        ...
        """
        try:
            cron_exist = self.env.ref('hc_shopify_ept.ir_cron_shopify_auto_import_order_instance_%d' % instance.id)
        except:
            cron_exist = False
        if self.shopify_order_auto_import:
            ...
            if cron_exist:
                ...
                core_cron = self.check_core_shopify_cron("hc_shopify_ept.ir_cron_shopify_auto_import_order")
    ...
    def setup_shopify_update_order_status_cron(self, instance):
        ...
        if self.shopify_order_status_auto_update:
            ...
            if cron_exist:
                ...
                core_cron = self.check_core_shopify_cron("hc_shopify_ept.ir_cron_shopify_auto_update_order_status")
    ...
    def setup_shopify_payout_auto_import_payout_report_cron(self, instance):
        ...
        if cron_exist:
            ...
            core_cron = self.check_core_shopify_cron("hc_shopify_ept.ir_cron_auto_import_payout_report")
    ...
    def setup_shopify_payout_auto_process_bank_statement_cron(self, instance):
        ...
        if cron_exist:
            ...
            core_cron = self.check_core_shopify_cron("hc_shopify_ept.ir_cron_auto_process_bank_statement")
    ...
    def action_shopify_open_cron_configuration_wizard(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "hc_shopify_ept.action_wizard_shopify_cron_configuration_ept")
