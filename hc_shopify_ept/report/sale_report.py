...
class SaleReport(models.Model):
    ...
    def shopify_sale_report(self):
        ...
        if version_info.get('server_version') == '14.0':
            action = self.env.ref('hc_shopify_ept.shopify_action_order_report_all').read()[0]
        else:
            action = self.env.ref('hc_shopify_ept.shopify_sale_report_action_dashboard').read()[0]
