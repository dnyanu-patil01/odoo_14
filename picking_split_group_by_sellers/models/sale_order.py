...
class SaleOrder(models.Model):
    ...
    def merge_quotations(self,order_ids):
        ...
        sorted_lines = sorted(self.browse(order_ids), key=itemgetter("shopify_order_number"))
        for shopify_order_number, order_value in groupby(sorted_lines, key=itemgetter("shopify_order_number")):
            all_order_lines = self.env['sale.order.line'].search([('seller_group_id','!=',False),('order_id','in',order_ids),('order_id.shopify_order_number','=',shopify_order_number)],order="seller_group_id")
