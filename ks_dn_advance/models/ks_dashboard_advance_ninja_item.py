        ...
        for rec in self:
            ...
            with api.Environment.manage():
                if rec.ks_data_calculation_type == 'query' and rec.ks_dashboard_item_type not in ['ks_tile', 'ks_kpi']\
                    ...
                    # - - - - - - - - - - - -
                    # START:
                    # Code added to replace
                    # ks_user_id from the custom query.
                    # - - - - - - - - - - - -
                    if 'ks_user_id' in ks_query:
                        user_id = self.env.user.id
                        if not user_id:
                            raise ValidationError(_('Unable to resolve user while running query which contains ks_user_id.'))
                        try:
                            # Please note here, this is a clever hack. Since the replacement of ks_start_date & ks_end_date is happening below.
                            # I have replaced those variables back with their original replacement strings for the code that comes below to be able to work.
                            # This was throwing an error otherwise trying to replace just ks_user_id.
                            ks_query = ks_query % {"ks_user_id": user_id, "ks_start_date": "%(ks_start_date)s", "ks_end_date": "%(ks_end_date)s"}
                        except Exception as e:
                            if type(e).__name__ == 'KeyError':
                                raise ValidationError(
                                    _('Wrong int variable, Please use ks_user_id with a "d" modifier in your custom query.'))
